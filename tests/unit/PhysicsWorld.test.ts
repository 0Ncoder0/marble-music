...
describe("PhysicsWorld", () => {
  ...
  it("PW-01: 仅 Ball 在步进中移动，Block 和 MusicBlock 位置不变", () => {
    ...
    const scene = makeScene([makeBall(ballId, 200, 50), makeBlock(blockId, 500, 50), makeMusicBlock(mbId, 800, 50)]);
  ...
  it("PW-03: Ball 碰撞 Block 后速度方向改变（y 方向由增大变为减小）", () => {
    ...
    const scene = makeScene([makeBall(ballId, 300, 164), makeBlock("block-1", 300, 200)]);
    world.loadScene(scene);
  ...
  it("PW-04: Ball 碰撞 MusicBlock 时输出包含正确 ballId 和 musicBlockId 的 CollisionEvent", () => {
    ...
    // 小球正上方，留 10px 间距
    const scene = makeScene([makeBall(ballId, 400, 164), makeMusicBlock(mbId, 400, 200)]);
    world.loadScene(scene);
