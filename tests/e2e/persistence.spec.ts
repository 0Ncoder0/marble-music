 ...
 * 返回放置时的大约画布坐标
 */
async function placeMusicBlock(
...
async function waitForSaved(page: import('@playwright/test').Page): Promise<void> {
  await page.waitForFunction(
    () => window.__debugState?.persistence?.saveStatus === 'saved',
    { timeout: 5_000 },
  );
}
...
test('E2E-20: 放置积木后刷新，位置和实体数量完整恢复', async ({ page }) => {
  ...
  // 等待实体放置
  await page.waitForFunction(
    () => (window.__debugState?.entityCount ?? 0) >= 3,
    { timeout: 3_000 },
  );
  ...
  // 记录实体信息
  const beforeEntities = await page.evaluate(() =>
    window.__debugState?.entities.filter((e) => e.kind === 'music-block') ?? [],
  );
...
test('E2E-22: 刷新后 Timeline 根据恢复场景自动重新预测', async ({ page }) => {
  ...
  // 等待预测完成
  await page.waitForFunction(
    () => (window.__debugState?.prediction?.noteCount ?? 0) > 0,
    { timeout: 5_000 },
  );

  const noteCountBefore = await page.evaluate(
    () => window.__debugState?.prediction?.noteCount ?? 0,
  );
  ...
  const noteCountAfter = await page.evaluate(
    () => window.__debugState?.prediction?.noteCount ?? 0,
  );
...
test('E2E-24: 高音量 voice 在 800ms 后仍大于 0（音量驱动衰减验证）', async ({ page }) => {
  ...
  // 记录碰撞后立即的 voice 数量
  const voiceCountImmediate = await page.evaluate(
    () => window.__debugState?.audioEngine?.activeVoiceCount ?? 0,
  );

  // 等待 800ms 后检查（volume=0.5 → 衰减约 1200ms，800ms 时应仍有 voice）
  await page.waitForTimeout(800);

  const voiceCountAfter800ms = await page.evaluate(
    () => window.__debugState?.audioEngine?.activeVoiceCount ?? 0,
  );
...
test('E2E-25: 极低音量 voice 约 300ms 后 activeVoiceCount 回到 0', async ({ page }) => {
  ...
  if (voiceCountImmediate > 0) {
    // 等待 voice 衰减完毕
    await page.waitForFunction(
      () => (window.__debugState?.audioEngine?.activeVoiceCount ?? 0) === 0,
      { timeout: 1_500 },
    );
    const voiceCountFinal = await page.evaluate(
      () => window.__debugState?.audioEngine?.activeVoiceCount ?? 0,
    );
