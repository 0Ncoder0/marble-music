...
test("E2E-17: camera follows selected ball after entering play mode", async ({ page }) => {
  ...
  // 验证场景中有球
  await page.waitForFunction(() => window.__debugState?.entities.some(e => e.kind === "ball"), { timeout: 3_000 });
...
test("E2E-19: scroll wheel changes zoom during play", async ({ page }) => {
  ...
  // 等待 RAF 处理 wheel 事件
  await page.waitForFunction(expectedZoom => (window.__debugState?.camera?.zoom ?? 1) > expectedZoom, initZoom, { timeout: 3_000 });
