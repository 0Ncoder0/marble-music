...
test("E2E-10: dragging entity in play mode does not move it", async ({ page }) => {
  ...
  await page.waitForFunction(() => window.__debugState?.entities.some(x => x.kind === "block"), { timeout: 3_000 });
...
test("E2E-12: panel-container and timeline-canvas are hidden in play mode", async ({ page }) => {
  ...
  const panelDisplayEdit = await page.locator("#panel-container").evaluate(el => window.getComputedStyle(el).display);
  expect(panelDisplayEdit).not.toBe("none");

  const timelineDisplayEdit = await page.locator("#timeline-canvas").evaluate(el => window.getComputedStyle(el).display);
  ...
  // 播放态：两个容器应被隐藏
  const panelDisplayPlay = await page.locator("#panel-container").evaluate(el => window.getComputedStyle(el).display);
  expect(panelDisplayPlay).toBe("none");

  const timelineDisplayPlay = await page.locator("#timeline-canvas").evaluate(el => window.getComputedStyle(el).display);
  expect(timelineDisplayPlay).toBe("none");

  // 返回编辑态：两个容器应重新可见
  await exitPlayMode(page);

  const panelDisplayBack = await page.locator("#panel-container").evaluate(el => window.getComputedStyle(el).display);
  expect(panelDisplayBack).not.toBe("none");

  const timelineDisplayBack = await page.locator("#timeline-canvas").evaluate(el => window.getComputedStyle(el).display);
...
test("E2E-12-panel: music block param panel visible in edit, hidden in play mode", async ({ page }) => {
  await page.goto("/");
  await waitForApp(page);
  ...
  const panelContainerDisplay = await page.locator("#panel-container").evaluate(el => window.getComputedStyle(el).display);
  expect(panelContainerDisplay).toBe("none");

  // 返回编辑态：panel-container 重新可见
  await exitPlayMode(page);

  const panelContainerBack = await page.locator("#panel-container").evaluate(el => window.getComputedStyle(el).display);
...
test("E2E-16: no pause button or pause text exists in UI", async ({ page }) => {
  ...
  // 也不应有英文 "pause"（不区分大小写）
  const pauseEnCount = await page.evaluate(() => document.body.innerText.toLowerCase().includes("pause"));
