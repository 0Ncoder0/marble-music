...
test('E2E-01/E2E-04: timeline shows predicted notes after placing ball and music block', async ({
  page,
}) => {
  await page.goto('/');
  ...
  const timelineDisplay = await page
...
test('E2E-08: deleting all music blocks results in empty staff (no notes, lines remain visible)', async ({
  page,
}) => {
  await page.goto('/');
  await waitForApp(page);
